...
public:
	MicrofacetGGX(const Properties &props)
		...
		m_ggx = djb::tabular::to_ggx(m_tabular);
	...
	Spectrum eval(const BSDFSamplingRecord &bRec, EMeasure measure) const {
		...
		djb::vec3 value = m_ggx->evalp(wo, wi);
	...
	Float PDF(const Vector& wm) const 
	{
		...
		return m_ggx->ndf(m) * Frame::cosTheta(wm);
	}
	...
	Spectrum sample(BSDFSamplingRecord &bRec, const Point2 &sample) const {
		...
		djb::vec3 wm = djb::vec3(m_ggx->sample(djb::dir(djb::vec3(bRec.wi.x, bRec.wi.y, bRec.wi.z)), sample.x, sample.y));
		...
		djb::vec3 fresnel = m_ggx->eval_fresnel(acosf(std::max<Float>(std::min<Float>(1,dot(bRec.wi, m)),0)));
