...
public:
	...
	dj_brdf(const Properties &props, EDistributionType distributionType = EBeckmann):
	BSDF(props) {
		...
		if (props.hasProperty("distribution")) {
			...
			if (distr == "beckmann")
				m_distributionType = EBeckmann;
			else if (distr == "ggx")
				m_distributionType = EGGX;
			else if (distr == "tabular")
				m_distributionType = ETabular;
			else
				SLog(EError, "Specified an invalid distribution \"%s\", must be "
					"\"beckmann\", \"ggx\", or \"tabular\"!", distr.c_str());
		}
		...
		if (props.hasProperty("merl")) { // fit a MERL BRDF
			...
			if (m_distributionType == ETabular) {
				m_brdf = tab;
			} else if (m_distributionType == EBeckmann) {
				...
				delete tab;
			} else if (m_distributionType == EGGX) {
				djb::microfacet::params params =
					djb::tabular::fit_ggx_parameters(*tab);

				/* save the roughness parameter */
				params.get_ellipse(&baseRoughness, &baseRoughness);
				m_brdf = new djb::ggx(tab->get_fresnel());
				delete tab;
			}
			...
			if (m_distributionType == ETabular) {
				...
				djb::microfacet::params params =
					djb::tabular_anisotropic::fit_beckmann_parameters(*tab);
				...
				djb::microfacet::params params =
					djb::tabular_anisotropic::fit_ggx_parameters(*tab);
			...
			if (m_distributionType == EBeckmann)
				m_brdf = new djb::beckmann();
			else if (m_distributionType == EGGX)
				m_brdf = new djb::ggx();
			else
				SLog(EError, "The tabular distribution requires a merl file !");
		}

		/* set up roughness */
		if (props.hasProperty("alpha")) {
			m_alpha1 =
			m_alpha2 = new ConstantFloatTexture(baseRoughness * props.getFloat("alpha", baseRoughness));
	...
	dj_brdf(Stream *stream, InstanceManager *manager)
	 : BSDF(stream, manager) {
 ...
 */
class dj_brdf_shader : public Shader {
public:
	dj_brdf_shader(Renderer *renderer, const Texture *specularReflectance,
...
Shader *dj_brdf::createShader(Renderer *renderer) const {
	return new dj_brdf_shader(renderer,
		m_specularReflectance.get(), m_alpha1.get(), m_alpha2.get(), m_eta, m_k);
}

MTS_IMPLEMENT_CLASS(dj_brdf_shader, false, Shader)
MTS_IMPLEMENT_CLASS_S(dj_brdf, false, BSDF)
MTS_EXPORT_PLUGIN(dj_brdf, "Rough conductor microfacet BRDF");
MTS_NAMESPACE_END
